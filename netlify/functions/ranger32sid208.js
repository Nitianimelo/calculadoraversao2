...
const DATA_PATH = path.resolve(
  ...
  '..',
  'script novos',
  'Script ranger 3.2 SID208',
  'sid208_ranger32_reset_regions.json'
);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!detected) {
      return json(422, {
        ok: false,
        error: 'Patch compativel nao encontrado para Ranger 3.2 SID208.',
    ...
    const baseName = decodeURIComponent(event.headers['x-file-name'] || 'ranger32_sid208.bin')
      .replace(/[\\/:*?"<>|]+/g, '_')
      .replace(/\.[^.]+$/, '');
