...
|---|------------|----------------------------|
| 1 | O **offset absoluto** do bloco de SW **muda entre versões de SW e entre motorizações**. Observado: `AVJ` em `0x277124`; `AVA`/`AVE` em `0x276E58`; `FJC` (2.2L) em `0x276EB0`. | **Proibido hardcodar offset.** A busca é sempre por padrão. |
...
| 3 | O `12K532` fica sempre em **+0x18 (24 bytes)** do início do `12A650`. Validado em 4 versões de SW (3.2L e 2.2L). | `SLOT = 0x18`. É o critério de confirmação do bloco. |
...
| 8 | O número de **HW varia por revisão de hardware** e **NÃO mapeia de forma confiável para motorização**. Observado dentro da *mesma* 2.2L: `AB39-12A650-DF` E `-DE`. O `AB3Q-12B684` também varia sem padrão limpo (`-AC` aparece em 3.2L E em 2.2L). | **Não derive motorização de nenhum campo isolado.** HW = allowlist de strings completas aceitas (sanity check). Classificação de motor, se necessária, é tabela de negócio externa, keyed na tupla completa de identificação e expandida empiricamente. |
| 9 | *Hipótese (não confirmada — 5 amostras):* a família do sufixo do SW de calibração correlaciona com motor — `AV*` → 3.2L, `FJ*` → 2.2L. | Pode servir de heurística, **nunca** de regra rígida. Não codifique como invariante. |
...
5. **HW contra allowlist (sanity check):** o campo HW deve existir e constar
   numa **allowlist configurável**, NÃO um valor único. HW observados até agora:
   `AB39-12A650-FE`, `-DF`, `-DE`. O HW é uma revisão de hardware e **não
   identifica motorização de forma confiável** (invariante #8) — não use o HW
   como chave de motor. HW fora da allowlist = central não suportada — rejeitar
   ou sinalizar conforme regra de negócio. **Não hardcode um único HW.**
...
Módulo testado em 9 arquivos reais — 5 versões de SW, 2 motorizações:

| Motor | Versão SW | Offset CAL | Offset SW | Gap | HW (`12A650`) |
|-------|-----------|-----------|-----------|-----|-----|
| 3.2L  | `AVJ`     | `0x277124`| `0x27713C`| `0x18` ✓ | `-FE` |
| 3.2L  | `AVA`     | `0x276E58`| `0x276E70`| `0x18` ✓ | `-FE` |
| 3.2L  | `AVE`     | `0x276E58`| `0x276E70`| `0x18` ✓ | `-FE` |
| 2.2L  | `FJC`     | `0x276EB0`| `0x276EC8`| `0x18` ✓ | `-DF` |
| 2.2L  | `FJG`     | `0x276EB0`| `0x276EC8`| `0x18` ✓ | `-DE` |

Padrão `12A650 + 12K532 @ +0x18` confirmado em 100% dos casos — única âncora
confiável. Offset absoluto **não** estável (varia por SW e por motor). O HW
varia por revisão de hardware e **não** identifica motorização (invariante #8):
duas 2.2L com HW distintos (`-DF`, `-DE`). Não derive motor de campo isolado.
